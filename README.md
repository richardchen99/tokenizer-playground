...
亲手训练一个基于 Unicode 字符的 BPE 分词器，检查每次合并背后的频次依据，再把学到的规则用于新文本。通过英文、中文、代码词汇与自定义语料，观察训练分布如何影响词表与切分结果。

[**进入实验室 ↗**](https://richardchen99.github.io/tokenizer-playground/) · [配套研究笔记](https://richardchen99.github.io/blog/tokenizer-playground-note/) · [English](README.en.md) · [本地运行](#本地运行)

作者：**Richard Chen · 中国人民大学** · [个人主页](https://richardchen99.github.io)

[![BPE 实验室：训练语料、相邻符号对频次、符号序列与已学规则](docs/assets/overview.jpg)](https://richardchen99.github.io/tokenizer-playground/)
...
| 实验 | 操作 | 可以观察到什么 |
| --- | --- | --- |
| **词表构建** | 播放、暂停或回退 | 字符如何逐步合并为更长的片段 |
| **符号对频次** | 查看频次最高的八个相邻符号对 | 为什么当前步骤选择这一对进行合并 |
| **自定义语料** | 编辑至多 1,200 字符，设置合并次数 | 训练分布怎样改变词表 |
| **新文本编码** | 使用当前已学规则切分文本 | 哪些片段能迁移到新词 |
| **无损还原检查** | 输入空格、换行或表情符号 | 是否精确还原原始输入 |

三组预设包括**经典英文词汇**（`low / lower / newest / widest`）、**中文研究词汇**（`研究 / 研究者 / 金融研究 / 决策智能`）与**代码词汇**（`token / tokenization / encoder / decoder`）。界面采用英文控件与中文解释，以下操作步骤保留控件原名，便于查找。

## 算法框架

![加权符号对统计与 BPE 训练，以及有序规则编码和文本还原流程](docs/assets/architecture.png)

*原创框架图：训练产生有序规则表，编码复用规则而不重新学习频次。[可编辑 SVG](docs/assets/architecture.svg) · [图片来源与状态](docs/assets/README.md)。*

## 复现前五次合并

1. 选择经典英文词汇（**Classic words**），逐步执行五次合并。
2. 查看历史：依次为 `e + s`（9）、`es + t`（9）、`est + </w>`（9）、`l + o`（7）、`lo + w`（7）。
3. 在文本输入框（**Input text**）输入 `lowest newer`，切换训练步数，观察某个片段何时成为可用规则。
4. 切换中文研究词汇（**Chinese research**），六次合并后，`金融研究者` 被切分为 `金融 / 研究 / 者`。
5. 尝试自定义语料 `aaa aaa aa a`：符号对统计包含重叠位置，实际替换从左向右进行且不重叠。

括号中的数字是选择该规则时的加权符号对频次，不简单等于这条规则执行的替换次数。
...
$f(w)$ 是词在语料中的频次， $N_w(a,b)$ 是当前符号序列中相邻符号对的出现次数，包含重叠位置。选中后执行从左向右、非重叠的替换，再重新统计频次。出现同频候选时，使用固定的英语区域排序规则确定顺序。

编码按**训练得到的有序规则**逐项执行，不在新句子中重新统计符号对。空白独立保留，未见字符保留原字符，以便无损还原。

这是以 **Unicode 码点**起步、按空白预分词、带内部 `</w>` 词尾标记的教学 BPE，不复现 GPT 的字节级 BPE、SentencePiece 或任何生产模型的分词器。输出词元数不能用于估算线上模型计费；保留标记 `</w>` 禁止出现在训练输入中。
...
| 入口 | 重点 |
| --- | --- |
| [`src/model.ts`](src/model.ts) | 词频、符号对统计、合并、编码与显示映射 |
| [`src/App.tsx`](src/App.tsx) | 语料编辑、训练状态、规则历史与新文本实验 |
| [`src/shared.tsx`](src/shared.tsx) · [`src/style.css`](src/style.css) | 词元动画、公式、玻璃面板与减少动态效果的无障碍支持 |
...
- Hugging Face：[Tokenizers](https://huggingface.co/docs/tokenizers/index)，生产分词器的组件与流程。
